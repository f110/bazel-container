...
func makeRepositoryMacroFile(outFile string) (*os.File, error) {
	...
	out.WriteString(`load("//private/deb:deb.bzl", "deb_pkg")
...
func makeMacroFile(outFile string, distros, packages []string, packageIndices map[string]*PackageIndex) error {
	...
	for _, distro := range distros {
		...
		for _, pkgName := range packages {
			...
			depPkgNames := Map(deps, func(t *Package) string { return t.Name })
			depPkgNames = Map(depPkgNames, func(v string) string { return fmt.Sprintf("%q", v) })
